...
x_keep <-select(x,-c("subjectid","Value.ADIP","Value.CRPM","Value.EIA-ESelectin","Value.EIA-IL-6","Value.EIA-SICAM","Value.EIA-TNF-A", 
                                          ...
                                          "Value.VLDL","Value.HDL","Value.TG-NET","Value.CHOL","DEXAPercFat","bmi","bmi_perc","eis"))
...
```{r echo=FALSE}
kable(t1b,caption = "Table 1b.  Descriptive statistics for participants with proteomics data and cytokine data.  Statistics given are N(%), mean±SD, or median (25th %ile, 75th %ile).")
```
\

```{r echo=FALSE}
kable(t1,caption = "Table 1c.  Descriptive statistics for participants with efflux data, by treatment group.  Statistics given are N(%), mean±SD, or median (25th %ile, 75th %ile). * denotes that the Mann-Whitney test was used, and ** denotes that Fisher's exact test was used.")
```
\
...
```{r echo=FALSE}
kable(corrcyto,caption = "Table 4.  Spearman correlation coefficients for changes in proteins with changes in cytokines.")
```
\

```{r echo=FALSE}
kable(effluxtab,caption = "Table 5.  Unadjusted changes in efflux, by treatment group.  Statistics given are mean±SD.")
```
\

```{r echo=FALSE}
kable(mod,caption = "Table 6.  Linear model of change in efflux by treatment group, adjusted for baseline HDL.  Statistics given are beta coefficients and 95% confidence intervals.")
```
